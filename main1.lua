...
local CustomDialog = require "mods.utils.CustomDialog"

-- 模型设置弹窗
模型设置.onClick = function()
    CustomDialog.show(activity, function(apiKey, apiUrl, selectedModel, ...)
        local args = {...} -- 获取可变参数
        local isDeepSeek = (apiUrl == "https://api.deepseek.com/chat/completions")
        ...
        if isDeepSeek then
            ...
            -- 硅基流动模式
            local maxTokens, temperature, topP, topK, frequencyPenalty, stream = table.unpack(args)
            print("使用硅基流动 API")
        end
        ...
        全局_apiKey = apiKey
        全局_apiUrl = apiUrl
        全局_model = selectedModel
        全局_isDeepSeek = isDeepSeek

        if not isDeepSeek then
            -- 硅基流动额外参数
            全局_maxTokens = maxTokens
            全局_temperature = temperature
            全局_topP = topP
            全局_topK = topK
            全局_frequencyPenalty = frequencyPenalty
            全局_stream = stream
        else
            全局_stream = stream
        end
...
发送.onClick = function()
    ...
    -- 记录对话
    写入信息(数据, {Who = "me", 内容 = inputText, 头像 = "png/me/7h.png"})
    写入信息(数据, {Who = "jiazai", 内容 = "", 头像 = "png/ta/deepseek.png"})

    local header = {
        ["Content-Type"] = "application/json",
        ["Authorization"] = "Bearer " .. 全局_apiKey
    }

    local data_body

    if 全局_isDeepSeek then
        -- DeepSeek 请求格式
        data_body = {
            stream = 全局_stream,
            model = "deepseek-chat",
            messages = 多轮对话组合("user", inputText)
        }
    else
        -- 硅基流动 请求格式
        data_body = {
            ...
            max_tokens = 全局_maxTokens,
            temperature = 全局_temperature,
            top_p = 全局_topP,
            top_k = 全局_topK,
            frequency_penalty = 全局_frequencyPenalty,
            n = 1
        }
    end
    ...
    Http.post(全局_apiUrl, json_body, cookie, charset, header, function(code, body)
        ...
        if code == 200 then
            local json = cjson.decode(body)
            local responseText = json.choices[1].message.content or "返回数据格式错误"

            写入信息(数据, {Who = "ta", 内容 = responseText, 头像 = "png/ta/deepseek.png"})
            多轮对话组合("assistant", responseText)
        else
            写入信息(数据, {Who = "ta", 内容 = "网络异常，请稍后再试\n" .. code, 头像 = "png/ta/deepseek.png"})
        end
